...
import com.korebap.app.view.function.Mkdir_file;
...
public class NoticeImageUploadController {
	...
	@ResponseBody
	public Map<String, String> uploadFile(@RequestBody MultipartFile upload, HttpSession session) {
		System.out.println("파일 업로드 시작");
		// 테스트용 멤버 ID 설정
		String member_id = "123@naver.com";
		...
		fileName = NoticeImageUploadController.img_security()+fileform;
        
        //저장할 폴더 명칭
        String folerName = "/board_img/"+member_id+"/";
        //가져온 파일을 저장할 위치를 지정해줍니다.
        System.out.println(servletContext);
        String uploadPath = servletContext.getRealPath(folerName);
        //저장될 위치를 확인하기 위한 로그
		...
		if(update_folder_session > 0) {
			...
			folder_num = NoticeImageUploadController.member_folder_num(uploadPath);
			//만약 저장되어 있는 개수가 없다면
			if(folder_session <= 0 && update_folder_session <= 0) {
				//사용자의 게시판 이미지 폴더 개수 확인용 로그
				System.out.println("사용자 게시판 이미지 폴더 확인용 로그 :"+folder_num);
				//번호를 불러와서 +1을 해준다음
				folder_num = NoticeImageUploadController.member_folder_num(uploadPath)+1;
				//폴더를 추가해줍니다.
        ...
        String fileUrl = servletContext.getContextPath() + folerName + folder_num + "/" + fileName;
