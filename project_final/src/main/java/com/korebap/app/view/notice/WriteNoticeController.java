...
public class WriteNoticeController {
	...
	public String writeNotice(BoardDTO boardDTO, FileDTO fileDTO, Model model, HttpSession session, ArrayList<FileDTO> fileList) {
		...
		if(folder_session > 0) {
			...
			for(String img_path : img_paths) {
				...
				try {
					...
					fileDTO.setFile_board_num(boardDTO.getBoard_num());
